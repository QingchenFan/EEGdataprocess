...
for train_index, test_index in kf.split(x_data):
    ...
    # Model
    # bagging,PLS
    bagging = BaggingClassifier(base_estimator=svm.SVC(kernel='rbf'))

    # 网格交叉验证
    cv_times = 2  # inner
    param_grid = {
        'base_estimator__C': [0.5, 1, 5, 10],
        'n_estimators': [8, 9]
    }
    predict_model = GridSearchCV(bagging, param_grid, scoring='accuracy', verbose=6, cv=cv_times)
